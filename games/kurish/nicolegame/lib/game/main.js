...
var MyGame = tpf.Game.extend({
	...
	init: function() {
		...
		//added april26 for music 
		
		ig.music.add('media/base1song.*', 'a');
		ig.music.volume = 0.2;
		ig.music.loop = true;
	...
	update: function() {
		...
		if( this.dead ) {
			...
			if( this.blobSpawnTimer.delta() > -.05 ) {
				this.spawnBlob();
			}
			if( this.blobSpawnTimer.delta() > -.06 ) {
				this.spawnTinki();
			}
			if( this.powerupSpawnTimer.delta() > -.03 ) {
				this.spawnLala();
			}
	...
	spawnTinki: function() {
		...
		this.spawnEntity(EntityEnemyTinkiSpawner, spawnPos.x, spawnPos.y);
	...
	spawnLala: function() {
		...
		this.spawnEntity(EntityEnemyLalaSpawner, spawnPos.x, spawnPos.y);
